 ...
 *               All Rights Reserved.
 **/
public class CategoryEditRunnable extends REBaseRunnable {
	...
	public CategoryEditRunnable(Handler handler, CategoryEntity entity) {
		super(handler);
		this.entity = entity;
	...
	public void run() {
		...
		if(REHttpUtility.isCheckConOK()) {
			String filePath = entity.getImgPath();
			int row = 1;
			// 允许图标为空
			if(filePath != null) {
			...
			if(bool) {
				...
				what = categoryService.update(entity);
			} else {
				what = -2; // 图片上传失败
			}
		} else {
